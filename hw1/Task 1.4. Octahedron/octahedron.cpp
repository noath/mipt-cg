...
glm::mat4 MoveCameraAroundZ(glm::mat4 Projection, glm::mat4 Model, float radius, float &angle, float delta = 1e-3f,
                            float x_bias = 0.0f, float y_bias = 0.0f, float z_bias = 0.0f) {
    auto camera_pos = glm::vec3(3);
    camera_pos[0] = radius * std::sin(angle) - x_bias; // X transformation
    camera_pos[1] = radius * std::cos(angle) - y_bias; // Y transformation
    camera_pos[2] = 0.0f - z_bias; // Z transformation (fixed)
...
int main(void) {
    ...
    GLuint programID = LoadShaders("3DTransformVertexShader.glsl", "ColorFragmentShader.glsl");
    ...
    do {
        ...
        if (current_angle < 2 * M_PI + M_PI / 2) {
            MVP = MoveCameraAroundY(Projection, Model, 5.0f, current_angle, M_PI / 360, 0.5f, 0.5f, 0.5f);
        } else if (current_angle < 4 * M_PI + M_PI / 2) {
            MVP = MoveCameraAroundX(Projection, Model, 5.0f, current_angle, M_PI / 360, 0.5f, 0.5f, 0.5f);
        } else {
